...
class FaceKeyPointData(Dataset):
    def __init__(self, csv_path='data/training_frames_keypoints.csv', split='training', device=torch.device('cpu'), model_input_size=224):
        ...
        self.split = split
        self.df = pd.read_csv(self.csv_path) #print(self.df)
        self.device = device
    ...
    def get_img(self, index):
        ...
        img = torch.tensor(img).permute(2, 0, 1) #img tensor represent as batchsize,channel,width,height but here width,height,channel so use permute
